...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (await hasValidWorldMonitorKey(worldMonitorKey)) {
    ...
    if (authHeader?.startsWith('Bearer ')) {
      ...
      // ponytail (task 08): no billing stack behind this deploy — any signed-in
      // session is treated as Pro. See docs/tasks/abdullah/08-server-entitlement-stripping.md.
      isPro = true;
    } else {
